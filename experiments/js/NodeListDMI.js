...
import * as highlighter from "../js/highlighter.js";
import * as outliner from "../js/outliner.js";
...
function calcCriticalPositions( header ) {
    ...
    for ( boxIndex = 0; boxIndex < boxes.length; boxIndex++ ) {
	computedStyle = getCStyle( boxes[boxIndex], null );
	boundingRect = boxes[boxIndex].getBoundingClientRect( );
...
function handleMousedown( event ) {
	...
	computedStyle = getCStyle( boxInMotion, null );
	if ( computedStyle.display == "inline" || computedStyle.display == "inline-block" ) {
...
function handleMousemove( event ) {
	...
	if ( targetBoxIndex == -1 ) {
	    ...
	    computedStyle = getCStyle( targetBox, null );
